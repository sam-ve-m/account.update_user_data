...
async def test_update_user_data_raising_errors(
        mocked_build_response, mocked_model, mocked_response_instance,
...
async def test_update_user_data(
        ...
        mocked_rules_application, mocked_validation_server_instance,
        mocked_validation, mocked_instance, mocked_jwt_decode, mocked_logger, monkeypatch,
):
    monkeypatch.setattr(flask, "request", MagicMock())
